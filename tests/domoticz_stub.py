...
class Unit:
    ...
    def Update(self, **kw):
        ...
        # nValue = 0 before writing. SwitchType is deliberately not modelled: maptypename
        # (PythonObjects.cpp, no "Ex") assigns it only in the switch-family branches, never in
        # "kwh" or "usage", so for the two names this stub covers the core leaves it untouched.
        # That is also what makes an in-place conversion safe, since a Usage device already
        # carries the 0 a kWh device wants.
